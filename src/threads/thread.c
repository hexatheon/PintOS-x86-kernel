...
static void init_thread1 (struct thread *, const char *name, int priority);
...
bool thread_cmp_wakeup_time (const struct list_elem *a, 
                             const struct list_elem *b, 
                             void *aux UNUSED) {
  return list_entry(a, struct thread, timer_elem)->wakeup_time < list_entry(b, struct thread, timer_elem)->wakeup_time;
}

void thread_join_wait_list(struct thread* t){
  list_insert_ordered (&wait_list, &t->timer_elem,
                       thread_cmp_wakeup_time, NULL);
}
void thread_wake_up_threads(int64_t ticks){
